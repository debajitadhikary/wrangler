...
import io.cdap.wrangler.api.RecipeContext;
 ...
 */
@PublicEvolving
public class AggregateStats implements Executor<Row> {
  ...
  @Override
  public void initialize(RecipeContext context) throws Exception {
  ...
  @Override
  public Row execute(Row row, ExecutorContext context) throws Exception {
    rowCount++;
    ...
    if (sizeValue instanceof Number) {
      ...
      if ("MB".equalsIgnoreCase(sizeUnit)) {
        totalSize += size * 1024 * 1024; // MB to bytes
      } else if ("KB".equalsIgnoreCase(sizeUnit)) {
        totalSize += size * 1024; // KB to bytes
      } else if ("GB".equalsIgnoreCase(sizeUnit)) {
        totalSize += size * 1024 * 1024 * 1024; // GB to bytes
      } else {
        totalSize += size; // Assume bytes
      }
    }
    ...
    if (timeValue instanceof Number) {
      ...
      if ("s".equalsIgnoreCase(timeUnit)) {
        totalTime += time * 1_000_000_000; // Seconds to nanoseconds
      } else if ("ms".equalsIgnoreCase(timeUnit)) {
        totalTime += time * 1_000_000; // Milliseconds to nanoseconds
      } else if ("min".equalsIgnoreCase(timeUnit)) {
        totalTime += time * 60 * 1_000_000_000; // Minutes to nanoseconds
      } else {
        totalTime += time; // Assume nanoseconds
      }
    }
  ...
  public void destroy() throws Exception {
    if (rowCount > 0) {
      ...
      double finalSize = totalSize / (1024 * 1024); // Bytes to MB
      double finalTime = totalTime / 1_000_000_000; // Nanoseconds to seconds
